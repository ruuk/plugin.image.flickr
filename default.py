...
def LOG(message):
    xbmc.log('plugin.image.flickr: %s' % ENCODE(str(message)), xbmc.LOGNOTICE)
...
class FlickrSession:
    ...
    def finishAuthenticate(self, token):
        ...
        rsp = self.flickr.test_login(format='xmlnode')

        self.user_id = rsp.user[0].attrib.get('id')
        self.username = rsp.user[0].username[0].text
