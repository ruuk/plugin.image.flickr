...
__plugin__ =  'flickrXBMC'
__author__ = 'ruuk'
__url__ = 'http://2ndmind.com'
__date__ = '09-8-2010'
__version__ = '0.5.0'
__settings__ = xbmcaddon.Addon(id='plugin.image.flickrxbmc')
...
class FlickrSession:
	...
	def PLACES(self,pid,woeid=None,name=''):
		...
		for p in places:
			count = p.get('count','0')
			#tn = "http://maps.google.com/maps/api/staticmap?center="+p.get('lat','0')+","+p.get('lon','0')+"&zoom=12&size=256x256&sensor=false&maptype=hybrid"
			#print tn
...
try:
	...
	if mode==None or url==None or len(url)<1:
		...
		fsession.PLACES(22,woeid=url,name=name)
	elif mode==1008:
		fsession.PLACES(7,woeid=url,name=name)
	elif mode==1012:
		fsession.PLACES(8,woeid=url,name=name)
except HTTPError,e:
	if(e.reason[1] == 504):
		dialog = xbmcgui.Dialog()
